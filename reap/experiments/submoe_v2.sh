...
dataset_name=${2:-"theblackcat102/evol-codealpaca-v1"}
compression_ratio=${3:-0.50}
server_log_file_name=${4:-"submoe_v2.log"}
port=${5:-8006}
seed=${6:-42}
# qa
run_lm_eval=${7:-true}
...
python src/reap/main.py \
    ...
    --merged-model-dir-name "submoeV2-${compression_ratio}" \
    --cluster-description "submoeV2" \
    --samples-per-category 1024 \
    --do-eval false

short_model_name=$(echo $model_name | cut -d'/' -f2)
short_dataset_name=$(echo $dataset_name | cut -d'/' -f2)
merged_model="artifacts/${short_model_name}/${short_dataset_name}/non_uniform_merged_models/submoeV2-${compression_ratio}/submoeV2"
